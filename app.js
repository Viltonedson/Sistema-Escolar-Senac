...
app.get('/comunicados', async (req, res) => {
    try {
        const comunicados = await Comunicados.find().populate('autor_id destinatarios'); // Popula os campos referenciados
...
app.post('/comunicados', async (req, res) => {
    const { titulo, conteudo, autor_id, destinatarios } = req.body;

    try {
        const comunicado = new Comunicados({
            titulo,
            conteudo,
            autor_id,
            destinatarios
        ...
        res.status(201).json({ message: 'Comunicado criado com sucesso!', comunicado });
    } catch (error) {
